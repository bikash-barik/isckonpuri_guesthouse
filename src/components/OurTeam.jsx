...
export default function OurTeam() {
  const [PopularChefData, setPopularChefData] = useState([]);
  const settings = {
    infinite: true,
    speed: 500,
    loop:true,
  ...
  useEffect(() => {
    ...
    const contentDataRef = firestore.collection("PopularChefData");
    const unsubscribe = contentDataRef.onSnapshot((snapshot) => {
      ...
      setPopularChefData(newContentData);
    });
    return () => unsubscribe();
  }, []);
        ...
        </div>
{
  <Slider {...settings} className="explorepurislider">
    ...
    {
      PopularChefData.map((item,index)=>(
        <>
        ...
        <div className="single_team">
                    ...
                    <div className="single_team_text">
                       ...
                       <button className="px-6 py-2 md:mt-2 mt-3 bg-orange-500 capitalize text-white rounded-md">
                       Buy Now
                       </button>
                    </div>
        </div>
